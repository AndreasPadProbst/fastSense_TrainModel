...
from utils.estimator import WordSenseEstimator, file_input_fn
...
class ModelTrainer:
    def __init__(self, model_dir: str, dataset_base_path: str, dataset_name: str, parameters: Dict[str, any]):
        ...
        if os.path.exists(self.data_descriptor_path) and os.path.exists(self.db_path):
            ...
            c = conn.execute("select count(*) from senses")  # retrieves number of rows from the column senses
            ...
            for train_file_path in glob.glob(file_pattern):
                for serializedExample in tf.io.tf_record_iterator(path=train_file_path, options=file_options):
        ...
        # session_config.gpu_options.per_process_gpu_memory_fraction = 0.333
    ...
    def export(self, export_dir_base: str):
        serving_input_receiver_fn = tf.estimator.export.build_raw_serving_input_receiver_fn(
            features={
                "tokens": tf.compat.v1.placeholder(tf.string, [None]),
                "possible_senses": tf.compat.v1.placeholder(tf.int64, [None])
            }
        )
        assets_extra = {
        ...
        return self.estimator.export_saved_model(
...
class TrainJob:
    def __init__(self, dataset_name: str, model_name: str, params: dict, epochs: int, batch_size: int,
                 train_sets: List[str], test_sets: List[str]):
    ...
    def run(self, job_runner: "TrainJobRunner"):
        ...
        if len(self.train_sets) > 0:
            ...
            for train_set in self.train_sets:
                print(
                    ("\033[0;34m" + "[{}-{}]" + "\033[m" + " Starting training...").format(self.model_name, train_set))
...
class AutoTrainJob:
    def __init__(self, dataset_name: str, model_name: str, params: dict, target: Dict[str, any], batch_size: int,
                 train_sets: List[str], test_sets: List[str]):
        self.dataset_name = dataset_name
        self.model_name = model_name
        self.params = params
    ...
    def run(self, job_runner: "TrainJobRunner"):
        ...
        while max_avg_slope is None or max_avg_slope >= self.end_if_slope_less_than:
            ...
            for test_set in self.test_sets:
                ...
                print(
                    ("\033[0;34m" + "[{}-{}]" + "\033[m" + " results = {}").format(self.model_name, test_set, results))
...
class TrainJobRunner:
    """
	Class for loading and running train and test jobs. Not intended for use outside of CLI script.
	"""
