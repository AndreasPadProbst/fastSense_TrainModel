...
def _parse_example(serialized_examples):
	examples = tf.io.parse_example(serialized_examples, features={
		...
		"possible_senses": tf.io.VarLenFeature(tf.int64),
		"sense": tf.io.FixedLenFeature([], tf.int64)
	})
...
def _model_fn(features, labels, mode, params, config):
	...
	embeddings = tf.compat.v1.get_variable(
		...
		dtype=tf.float32,
		initializer=tf.initializers.GlorotUniform(seed=tf_random_seed),
		trainable=True
	)
	...
	for layer_index in range(len(hidden_layer_sizes)):
		...
		with tf.compat.v1.variable_scope("layer{:d}_".format(layer_index + 1)):
			l_weights = tf.compat.v1.get_variable(
				name="weights",
			...
			l_biases = tf.compat.v1.get_variable(
				name="biases",
	...
	out_weights_transposed = tf.compat.v1.get_variable(
		name="out_weights_transposed",
		shape=[number_of_senses, prev_layer_size],
		dtype=tf.float32,
		initializer=tf.initializers.GlorotUniform(seed=(tf_random_seed + 2) if tf_random_seed is not None else None),
		trainable=True
	)

	out_biases = tf.compat.v1.get_variable(
		name="out_biases",
	...
	if mode == tf.estimator.ModeKeys.PREDICT:
		tokens = tf.strings.to_hash_bucket_fast(features["tokens"], num_buckets=hash_bucket_size)
		possible_senses = features["possible_senses"]

		embedded_tokens = tf.nn.embedding_lookup(embeddings, tokens)

		if use_sqrtn_combiner:
			embedded_tokens = tf.math.reduce_sum(embedded_tokens, axis=0) / tf.math.sqrt(tf.cast(tf.size(tokens), tf.float32))
		else:
			embedded_tokens = tf.math.reduce_mean(embedded_tokens, axis=0)

		embedded_tokens_batch = tf.expand_dims(embedded_tokens, axis=0)

		for w, b in zip(hidden_layer_weights, hidden_layer_biases):
			embedded_tokens_batch = tf.linalg.matmul(embedded_tokens_batch, w) + b
		...
		relative_prediction = tf.math.argmax(out_layer_logits)
		...
		tokens_batch = tf.SparseTensor(
			...
			values=tf.strings.to_hash_bucket_fast(features["tokens_values"], num_buckets=hash_bucket_size),
			dense_shape=features["tokens_dense_shape"]
		)
		possible_senses_batch = tf.sparse.SparseTensor(
		...
		relative_senses_batch = tf.math.argmax(
			tf.cast(
				tf.math.equal(
		...
		mask = tf.math.segment_sum(relative_possible_senses_one_hot, segment_ids)

		masked_out_layer = out_layer + (tf.math.reduce_max(out_layer) - tf.math.reduce_min(out_layer) + 1.0) * mask
		relative_predictions = tf.math.argmax(masked_out_layer, axis=1)

		if mode == tf.estimator.ModeKeys.TRAIN:
			global_step_var = tf.compat.v1.train.get_global_step()

			if decay_rate != 1.0:
				actual_learning_rate = tf.compat.v1.train.exponential_decay(
			...
			optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=actual_learning_rate)
			...
			accuracy = tf.math.reduce_mean(correct_predictions)
			...
			mean_per_class_acc = tf.compat.v1.metrics.mean_per_class_accuracy(
				labels=senses_batch,
